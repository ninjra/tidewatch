...
with 542 deterministic and property-based tests---including 23
...
(EDF), FIFO, and random baselines.  At $N{=}50$, Tidewatch matches EDF on missed-deadline
rates; at $N{=}200$, EDF outperforms Tidewatch on missed
deadlines ($18.5\%$ vs.\ $20.1\%$), reflecting the cost of
multi-factor ranking against a deadline-only optimizer.
...
standard library.
The core library comprises 1{,}740 lines across seven modules:
...
\midrule
\texttt{pressure.py}   & 575 \\
\texttt{types.py}      & 297 \\
\texttt{components.py} & 242 \\
\texttt{constants.py}  & 236 \\
\texttt{planner.py}    & 230 \\
\texttt{triage.py}     & 114 \\
\texttt{\_\_init\_\_.py} & 46 \\
\midrule
\textbf{Total}         & \textbf{1{,}740} \\
\bottomrule
\end{tabular}
\end{table}
...
The full test suite comprises 542 tests organized into 21
...
Values are mean $\pm$ 1 SD [95\% bootstrap CI].
Zero inversions for Tidewatch is a design property
(see text), not an empirical finding.}
\label{tab:mc50}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Strategy} & \textbf{Missed} & \textbf{Inv.} &
  \textbf{Eff.} \\
\midrule
Tidewatch          & $.067 {\scriptstyle\pm .011}$ {\tiny[.065,.068]} & $.000$ & $.931$ \\
TW+BW ($b{=}0.5$) & $.064 {\scriptstyle\pm .010}$ {\tiny[.063,.066]} & $.042$ & $.932$ \\
TW+BW ($b{=}0.2$) & $.064 {\scriptstyle\pm .010}$ {\tiny[.063,.066]} & $.080$ & $.932$ \\
TW+BW (var.)       & $.064 {\scriptstyle\pm .010}$ {\tiny[.063,.066]} & $.057$ & $.932$ \\
Weighted-EDF       & $.067 {\scriptstyle\pm .011}$ {\tiny[.065,.068]} & $.133$ & $.931$ \\
Weighted-sum       & $.116 {\scriptstyle\pm .017}$ {\tiny[.113,.118]} & $.299$ & $.902$ \\
EDF                & $.067 {\scriptstyle\pm .011}$ {\tiny[.065,.068]} & $.134$ & $.931$ \\
FIFO               & $.127 {\scriptstyle\pm .010}$ {\tiny[.126,.128]} & $.505$ & $.897$ \\
Random             & $.130 {\scriptstyle\pm .022}$ {\tiny[.127,.133]} & $.495$ & $.895$ \\
\bottomrule
\end{tabular}
\end{table}
...
Values are mean $\pm$ 1 SD [95\% bootstrap CI].  Saturation rate:
...
\textbf{Strategy} & \textbf{Missed} & \textbf{Inv.} &
  \textbf{Eff.} & \textbf{Sat.} \\
\midrule
Tidewatch           & $.201 {\scriptstyle\pm .018}$ {\tiny[.199,.204]} & $.000$ & $.818$ & $9\%$ \\
TW+BW ($b{=}0.5$)  & $.200 {\scriptstyle\pm .017}$ {\tiny[.198,.202]} & $.036$ & $.816$ & $9\%$ \\
TW+BW (var.)        & $.199 {\scriptstyle\pm .017}$ {\tiny[.196,.201]} & $.051$ & $.815$ & $9\%$ \\
Weighted-EDF        & $.186 {\scriptstyle\pm .014}$ {\tiny[.184,.188]} & $.097$ & $.838$ & $9\%$ \\
Weighted-sum        & $.320 {\scriptstyle\pm .015}$ {\tiny[.318,.322]} & $.256$ & $.696$ & $9\%$ \\
EDF                 & $\mathbf{.185} {\scriptstyle\pm .015}$ {\tiny[.183,.187]} & $.099$ & $\mathbf{.837}$ & $9\%$ \\
FIFO                & $.319 {\scriptstyle\pm .013}$ {\tiny[.317,.321]} & $.495$ & $.692$ & $9\%$ \\
Random              & $.312 {\scriptstyle\pm .024}$ {\tiny[.309,.316]} & $.498$ & $.696$ & $9\%$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Key findings.}
Tidewatch's zero queue-inversion rate ($0.0\%$) is a \emph{design
property}, not an empirical finding: processing obligations in
pressure-descending order cannot, by construction, invert the
pressure ranking.  We report it for completeness but emphasize that
EDF's $10$--$13\%$ inversion rate measures inversions on
\emph{Tidewatch's metric}, not on EDF's own deadline-ordering
objective.  Each strategy achieves zero inversions on its own
criterion.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Gaming:} self-reported signals are gameable.  The
    system does not currently verify signal provenance; integration
    with authenticated wearable APIs would mitigate this.
\end{itemize}
...
With 542 deterministic tests and zero runtime dependencies,
