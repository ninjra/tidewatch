...
factors combined by product: exponential time-decay, materiality
weighting, temporally gated dependency fanout, logistic completion
dampening, timing-sensitivity amplification, and deadline-violation
amplification with exponential decay.  These factors are retained in
a six-dimensional component space---deferred scalarization---that
supports both Pareto-dominance ranking and weighted aggregation,
deferring the combination of factors until ranking time and
preserving per-factor interpretability.  A specified but
empirically unvalidated capacity-aware reranking extension
incorporates operator state signals via a three-tier risk policy.

We evaluate the core scoring engine with 563 deterministic and
property-based tests (including 23 Hypothesis-driven monotonicity
and bounds checks, with golden values derived from closed-form
equation solutions) and Monte Carlo scheduling simulations (200
trials, seed${=}42$, LogNormal task durations; full parameterization
in Section~5).  EDF is theoretically optimal for deadline
minimization.  Tidewatch does not compete on that metric: at
$N{=}50$, it matches EDF ($6.7\%$ missed deadlines each); at
$N{=}200$, EDF achieves $18.5\%$ vs.\ Tidewatch's $20.1\%$---an
$8.6\%$ relative cost of encoding materiality, dependency fanout,
and completion state alongside deadline proximity.
The primary contribution is interpretable, auditable multi-factor
ranking with deferred scalarization, not deadline optimality.
All scoring functions are verified to $\Delta < 10^{-10}$ against
analytically computed reference values under pinned Python~3.11+
with zero runtime dependencies, establishing deterministic
reproducibility within a controlled execution environment.
No field deployment, user study, or real physiological dataset
is presented; external validity remains an open question.
\end{abstract}
