...
$200$), Tidewatch maintains comparable missed-deadline
...
product collapse outperforms normalized weighted-sum MCDM on
missed deadlines.  The dependency temporal gate uses an independent
...
The implementation comprises 1{,}704 lines of pure-stdlib Python
with zero runtime dependencies.  All scoring functions ship with
...
standard library.
The core library comprises 1{,}704 lines across seven modules:
...
Values are mean $\pm$ one standard deviation across trials.
Bandwidth reranking at $b{=}0.5$ slightly \emph{improves} deadline
compliance.  Weighted-sum MCDM underperforms product collapse on
all metrics.  Zero inversions for Tidewatch is a design property
...
Values are mean $\pm$ one standard deviation.  95\% bootstrap CIs
in brackets.  Weighted-EDF is a hybrid baseline combining zone-based
urgency tiers with deadline ordering.  Saturation rate: fraction of
obligations at $P \geq 0.999$.}
\label{tab:mc200}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Strategy} & \textbf{Missed} & \textbf{Inversions} &
  \textbf{Efficiency} & \textbf{Sat.} \\
\midrule
Tidewatch          & $0.201 \pm 0.018$ & $0.000$ &
  $0.818$ & $9.0\%$ \\
TW+BW ($b{=}0.5$) & $0.200 \pm 0.017$ & $0.036$ &
  $0.816$ & $9.0\%$ \\
Weighted-EDF       & $0.186 \pm 0.014$ & $0.097$ &
  $0.838$ & $9.0\%$ \\
Weighted-sum       & $0.320 \pm 0.015$ & $0.256$ &
  $0.696$ & $9.0\%$ \\
EDF                & $\mathbf{0.185 \pm 0.015}$ & $0.099$ &
  $\mathbf{0.837}$ & $9.0\%$ \\
FIFO               & $0.319 \pm 0.013$ & $0.495$ &
  $0.692$ & $9.0\%$ \\
Random             & $0.312 \pm 0.024$ & $0.498$ &
  $0.696$ & $9.0\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Gravity tiebreak placeholder.}
        The $g_w \cdot s_g$ term in Equation~\ref{eq:fit} is a
        placeholder for integration with the Gravitas coherence
        scoring system.  Currently $s_g$ defaults to $0$ (no effect)
        unless the caller provides a pre-computed gravity score.
        This term is explicitly zeroed in all evaluation results
        reported in this paper; it does not affect any claim.
\end{enumerate}
...
At 1{,}704 lines with zero dependencies and 541 deterministic tests,
