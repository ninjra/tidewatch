...
with 560 deterministic and property-based tests---including 23
...
(EDF), FIFO, and random baselines.  At $N{=}50$, Tidewatch matches
EDF on missed-deadline rates ($6.7\%$ each); at $N{=}200$, EDF
outperforms pure Tidewatch on missed deadlines ($18.5\%$ vs.\
$20.1\%$), reflecting the cost of multi-factor ranking against a
deadline-only optimizer---though bandwidth-adjusted Tidewatch
($17.8$--$18.2\%$) closes this gap.
Against na\"{\i}ve baselines (FIFO, Random), Tidewatch reduces
missed deadlines by $35$--$47\%$.  The dependency temporal gate
uses an independent
rate constant ($k_f = 2.0$) decoupled from base time pressure
($k = 3.0$), the timing amplifier uses a logistic ramp replacing
the prior step function, and violation amplification decays with a
14-day half-life to prevent perverse feedback loops.
All scoring functions ship with
golden-value regression tests verified to $\Delta < 10^{-10}$,
establishing deterministic reproducibility as a first-class design
constraint.
\end{abstract}
...
standard library.
The core library comprises 1{,}846 lines across seven modules:
...
\midrule
\texttt{pressure.py}   & 643 \\
\texttt{types.py}      & 297 \\
\texttt{components.py} & 270 \\
\texttt{constants.py}  & 246 \\
\texttt{planner.py}    & 230 \\
\texttt{triage.py}     & 114 \\
\texttt{\_\_init\_\_.py} & 46 \\
\midrule
\textbf{Total}         & \textbf{1{,}846} \\
\bottomrule
\end{tabular}
\end{table}
...
The full test suite comprises 560 tests organized into 21
...
TW+BW ($b{=}0.5$) & $.060 {\scriptstyle\pm .000}$ & $.036$ & $.935$ \\
TW+BW ($b{=}0.2$) & $.060 {\scriptstyle\pm .000}$ & $.076$ & $.935$ \\
TW+BW (var.)       & $.060 {\scriptstyle\pm .001}$ & $.053$ & $.935$ \\
...
TW+BW ($b{=}0.5$)  & $.182 {\scriptstyle\pm .019}$ & $.048$ & $.827$ & $9\%$ \\
TW+BW ($b{=}0.2$)  & $.171 {\scriptstyle\pm .018}$ & $.076$ & $.831$ & $9\%$ \\
TW+BW (var.)        & $.178 {\scriptstyle\pm .019}$ & $.060$ & $.828$ & $9\%$ \\
...
$N{=}200$: TW+BW ($b{=}0.5$) achieves $18.2\%$ and TW+BW (var.)
achieves $17.8\%$, both outperforming pure EDF ($18.5\%$).  This
...
TW+BW (var.) achieves $17.8\%$ missed deadlines, outperforming
...
With 560 deterministic tests and zero runtime dependencies,
