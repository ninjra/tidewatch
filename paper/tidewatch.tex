...
% ======================================================================
\begin{abstract}
Conventional task managers reduce deadlines to binary states---overdue
or not---discarding the continuous accumulation of risk and ignoring
whether the user's current cognitive capacity permits execution of
high-demand work.  We present Tidewatch, a multi-objective
task-prioritization framework that decomposes urgency into six
multiplicative factors: exponential time-decay, materiality weighting,
temporally gated dependency fanout, logistic completion dampening
(urgency decays as completion approaches 100\%), timing-sensitivity
amplification (continuous escalation for stagnant tasks via logistic
ramp), and deadline-violation amplification (with exponential decay
to prevent unbounded urgency escalation on chronically delayed tasks).
These factors are maintained in a Late Collapse
representation---a six-dimensional component space supporting
Pareto-dominance ranking and deferred scalarization---enabling
multi-criteria prioritization without premature aggregation.

An optional capacity-aware reranking stage incorporates
operator state signals (e.g., sleep quality, pain level, HRV trend)
to adapt task presentation under reduced bandwidth.  We specify and
implement this mechanism with a three-tier risk policy; empirical
validation against real physiological data remains future work.

We evaluate Tidewatch via 563 deterministic and property-based
tests---including 23 Hypothesis-driven monotonicity and bounds
checks---and Monte Carlo scheduling simulations (200 trials,
seed${}=42$, LogNormal task durations) against earliest-deadline-first
(EDF), weighted-EDF, FIFO, and random baselines.
EDF is theoretically optimal for deadline minimization; Tidewatch does
not claim to beat it on that metric.  At $N{=}50$, Tidewatch matches
EDF on missed-deadline rate ($6.7\%$ each); at $N{=}200$, EDF
achieves $18.5\%$ vs.\ Tidewatch's $20.1\%$, reflecting the
cost of encoding materiality, dependencies, and completion alongside
deadline proximity.  Capacity-adjusted variants narrow this gap
($19.5$--$20.0\%$).  Relative to na\"{\i}ve baselines, Tidewatch
reduces missed deadlines by $35$--$47\%$.

The primary contribution is not deadline optimality but interpretable,
auditable multi-factor ranking with a formally deferred scalarization
architecture.  All scoring functions are verified to
$\Delta < 10^{-10}$ via golden-value regression under pinned
Python~3.11+ and stdlib-only dependencies, establishing deterministic
reproducibility within a controlled execution environment.
\end{abstract}
...
Tidewatch is a \emph{prioritization heuristic}, not a scheduler.
...
We evaluate it against scheduling baselines (EDF, FIFO) to
demonstrate that a multi-factor prioritization heuristic can achieve
deadline compliance comparable to scheduling-theoretic optima while
encoding richer input dimensions.
...
(urgent/important).  ICE~\citep{ice2016scoring} and
RICE~\citep{rice2018intercom} score by impact, confidence, effort,
and reach---additive heuristics used in product management.
Weighted Shortest Job First (WSJF)~\citep{reinertsen2009flow}
divides cost-of-delay by duration.  All are static: they compute
a one-time score without modeling temporal dynamics, completion
progress, or operator capacity.  Tidewatch adds continuous
time-decay, multiplicative factor interaction, and
bandwidth-adaptive reranking.

\textbf{Multi-criteria decision making (MCDM).}
Weighted-sum aggregation, TOPSIS~\citep{hwang1981topsis}, and
PROMETHEE~\citep{brans1985promethee} are standard approaches for
combining multiple scored dimensions into a single ranking.
AHP~\citep{saaty1980ahp} derives weights from pairwise preference
judgments.  Tidewatch's product collapse differs from additive MCDM
methods: it treats factors as multiplicative modulators of a base
signal rather than interchangeable dimensions, preventing a high
score on one factor from compensating for zero urgency on another.
The weighted-sum MCDM baseline in Section~\ref{sec:monte_carlo}
uses normalized equal weights after scaling each component to $[0,1]$
by its algebraic bounds.
...
discrimination above the ceiling, but the default scalar pressure
does not.
...
With 563 deterministic tests and zero runtime dependencies,
Tidewatch demonstrates that useful prioritization infrastructure
does not require complexity---it requires the right equation.
